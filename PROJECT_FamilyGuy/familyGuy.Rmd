...
family_POS <- family_tidy %>% 
  inner_join(parts_of_speech) %>% 
  mutate(pos = factor(pos))

# Visual
family_POS %>% head(10) %>% 
  knitr::kable(caption = "Family Tidy")
```
...
# Season
lda_Season_2_a0 = LDA(x = family_dtm_Season, k = 2, 
                      method = "Gibbs", 
                      control = list(alpha = 0, delta = 0.1, seed = 123))

lda_Season_2_a1 = LDA(x = family_dtm_Season, k = 2, 
                   method = "Gibbs", 
                   control = list(alpha = 1, delta = 0.1, seed = 123))

lda_Season_2_a3 = LDA(x = family_dtm_Season, k = 2, 
                        method = "Gibbs", 
                        control = list(alpha = 3, delta = 0.1, seed = 123))

# Character
lda_Character_2_a0 = LDA(x = family_dtm_Character, k = 2, 
                         method = "Gibbs", 
                         control = list(alpha = 0, delta = 0.1, seed = 123))


lda_Character_2_a1 = LDA(x = family_dtm_Character, k = 2, 
                      method = "Gibbs", 
                      control = list(alpha = 1, delta = 0.1, seed = 123))

lda_Character_2_a3 = LDA(x = family_dtm_Character, k = 2, 
                           method = "Gibbs", 
                           control = list(alpha = 3, delta = 0.1, seed = 123))

```

```{r lda_2_topics_Season, warning=FALSE, message=FALSE, echo=FALSE}

# Season
gg_lda_2_a0_Season <- lda_Season_2_a0 %>% tidy(matrix = "gamma") %>% 
  mutate(
    topic = topic %>% factor,
  ...
  geom_line() +
  scale_x_continuous(n.breaks = 11, limits = c(1,12)) +
  labs(
    subtitle = "alpha = 0",
    x = NULL
  ) +
  theme(
    plot.subtitle = element_text(hjust = 0.5, face = "italic", size = 15, color = "red"),
    axis.text.x = element_text(face = "bold", size = 12, color = "darkgoldenrod"),
    legend.position = "none"
  )

gg_lda_2_a1_Season <- lda_Season_2_a1 %>% tidy(matrix = "gamma") %>% 
  mutate(
    topic = topic %>% factor,
    document = as.integer(document)
  ) %>% 
...
gg_lda_2_a3_Season <- lda_Season_2_a3 %>% tidy(matrix = "gamma") %>% 
  mutate(
    topic = topic %>% factor,
    document = as.integer(document)
  ) %>% 
...
```{r lda_2_topics_Season_VIZ, warning=FALSE, message=FALSE, echo=FALSE, fig.align='center', fig.height=5, fig.width=15}

# Visual
ggarrange(gg_lda_2_a0_Season, gg_lda_2_a1_Season, gg_lda_2_a3_Season, nrow = 1)
```

```{r lda_2_topics_Character, warning=FALSE, message=FALSE, echo=FALSE}
# Character
gg_lda_2_a0_Character <- lda_Character_2_a0 %>% tidy(matrix = "gamma") %>%
  mutate(
...
gg_lda_2_a1_Character <- lda_Character_2_a1 %>% tidy(matrix = "gamma") %>%
  mutate(
    document = factor(document),
    topic = factor(topic)
  ) %>% 
...
gg_lda_2_a3_Character <- lda_Character_2_a3 %>% tidy(matrix = "gamma") %>%
  mutate(
    document = factor(document),
    topic = factor(topic)
  ) %>% 
...
```{r lda_2_topics_Character_VIZ, warning=FALSE, message=FALSE, echo=FALSE, fig.align='center', fig.height=5, fig.width=15}

# Visual 
ggarrange(gg_lda_2_a0_Character, gg_lda_2_a1_Character, gg_lda_2_a3_Character, nrow = 1)
```

```{r lda_2_words_Season, warning=FALSE, message=FALSE, echo=FALSE}

# Season
gg_lda_2_a0__word_Season <- lda_Season_2_a0 %>% 
  tidy(matrix = "beta") %>% 
...
gg_lda_2_a1__word_Season <- lda_Season_2_a1 %>% 
  tidy(matrix = "beta") %>% 
  mutate(topic = factor(topic)) %>% 
  group_by(topic) %>% 
  top_n(n = 10, wt = beta) %>% 
...
gg_lda_2_a3__word_Season <- lda_Season_2_a3 %>% 
  tidy(matrix = "beta") %>% 
  mutate(topic = factor(topic)) %>% 
  group_by(topic) %>% 
  top_n(n = 10, wt = beta) %>% 
...
```{r lda_2_words_Season_VIZ, warning=FALSE, message=FALSE, echo=FALSE, fig.align='center', fig.height=5, fig.width=15}

# Visual
ggarrange(gg_lda_2_a0__word_Season, gg_lda_2_a1__word_Season, gg_lda_2_a3__word_Season, nrow = 1)
```

```{r lda_2_words_Character, warning=FALSE, message=FALSE, echo=FALSE}

# Character
gg_lda_2_a0__word_Character <- lda_Character_2_a0 %>% 
  tidy(matrix = "beta") %>% 
  mutate(topic = factor(topic)) %>% 
  group_by(topic) %>% 
  top_n(n = 10, wt = beta) %>% 
...
gg_lda_2_a1__word_Character <- lda_Character_2_a1 %>% 
  tidy(matrix = "beta") %>% 
  mutate(topic = factor(topic)) %>% 
  group_by(topic) %>% 
  top_n(n = 10, wt = beta) %>% 
...
gg_lda_2_a3__word_Character <- lda_Character_2_a3 %>% 
  tidy(matrix = "beta") %>% 
  mutate(topic = factor(topic)) %>% 
  group_by(topic) %>% 
  top_n(n = 10, wt = beta) %>% 
...
```{r lda_2_words_Character_VIZ, warning=FALSE, message=FALSE, echo=FALSE, fig.align='center', fig.height=5, fig.width=15}

# Visual
ggarrange(gg_lda_2_a0__word_Character, gg_lda_2_a1__word_Character, gg_lda_2_a3__word_Character, nrow = 1)
...
# Quanteda [TABS] {.tabset .tabset-fade}

## Corpus

```{r corpus, warning=FALSE, message=FALSE, echo=FALSE}
family_corpus <- corpus(family, text_field = "dialog")

docid <- paste(family$index,
               family$character,
               sep = "-")

docnames(family_corpus) <- docid

family_corpus %>% summary()
```

## Tokens

```{r tokens, warning=FALSE, message=FALSE, echo=FALSE}
family_tokens <- family_corpus %>% tokens(remove_punct = TRUE) 
family_tokens %>% kwic(pattern = "damn*", window = 10)
family_tokens %>% kwic(pattern = phrase("peter griffin"))
family_tokens_nostops <- family_tokens %>% tokens_remove(pattern = stopwords("en"))

# n-grams
family_tokens_nostops %>% 
...
```{r dfm, warning=FALSE, message=FALSE, echo=FALSE}
family_dfm <- family_tokens_nostops %>% dfm()
family_dfm %>% topfeatures(10)

# tfidf
family_tfidf <- family_dfm %>% dfm_tfidf()
family_tfidf %>% topfeatures(10)
```

```{r dfm_select, warning=FALSE, message=FALSE, echo=FALSE}
family_dfm %>% dfm_remove(pattern = stopwords("en"))
family_dfm %>% dfm_trim(min_termfreq = 10)
```

```{r dfm_group, warning=FALSE, message=FALSE, echo=FALSE}
family_dfm_Season <- family_dfm %>% dfm_group(groups = season)
family_dfm_Character <- family_dfm %>% dfm_group(groups = character)
```
